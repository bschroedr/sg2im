...
class Sg2ImModel(nn.Module):
  def __init__(self, vocab, image_size=(64, 64), embedding_dim=64,
               ...
               masks_to_triplet_mlp = False,
               masks_to_triplet_pixels = False,
               **kwargs):
    ...
    self.masks_to_triplet_mlp = masks_to_triplet_mlp
    self.masks_to_triplet_pixels = masks_to_triplet_pixels
    ...
    # extreme points
    extreme_net_dim = 10   #8+2 #centers
    extreme_net_layers = [gconv_dim,  gconv_hidden_dim, extreme_net_dim]
    self.extreme_net = build_mlp(extreme_net_layers, batch_norm=mlp_normalization)
    ...
    # use masks as triplet priors
    self.prior_net = None
    if self.triplet_mask_size > 0:
      if self.masks_to_triplet_mlp and self.prior_net is None:
        # pass in object mask (mask_size*mask_size), output 128D embedding
        prior_net_layers = [mask_size*mask_size, gconv_hidden_dim, gconv_dim]
        self.prior_net = build_mlp(prior_net_layers, batch_norm=mlp_normalization)
        # input is subject/object mask mlp embeddings (prior) + triplet embedding
        self.triplet_mask_net = self._build_triplet_mask_net(num_objs, 3*gconv_dim + 2*gconv_dim, self.triplet_mask_size)
      elif self.masks_to_triplet_pixels:
        # input is raw pixels subject/object masks (2*mask_size*mask_size + triplet embedding (3*128)
        self.triplet_mask_net = self._build_triplet_mask_net(num_objs, 3*gconv_dim + 2*mask_size*mask_size, self.triplet_mask_size) 
      else:
        # input dimsn is 3*128 for concatenated triplet, output dimsn is triplet_mask_size
        # EP: input dimsn is 2*10 + 3*128 for concatenated EP + triplet, output dimsn is triplet_mask_size
        self.triplet_mask_net = self._build_triplet_mask_net(num_objs, 20 + 3*gconv_dim, self.triplet_mask_size)
    ...
    if mask_size is not None and mask_size > 0:
      # EP: 10 + 128 input dimn 
      self.mask_net = self._build_mask_net(num_objs, 10 + gconv_dim, mask_size)
  ...
  def forward(self, objs, triples, obj_to_img=None,
    ...
    # bounding box prediction (for EP, predict but don't use)
    boxes_pred_info = None
    if self.use_bbox_info:
      # bounding box prediction + augmented box info
      boxes_pred_info = self.box_net(obj_vecs)
      boxes_pred = boxes_pred_info[:,0:4] # first 4 entries are bbox coords 
    else:
      boxes_pred = self.box_net(obj_vecs) 

    # predict extreme points
    extreme_pts_pred = self.extreme_net(obj_vecs)

    # singleton mask net
    masks_pred = None
    layout_masks = None
    if self.mask_net is not None:
      mask_input = torch.cat([extreme_pts_pred, obj_vecs], dim=1)
      mask_scores = self.mask_net(mask_input.view(O, -1, 1, 1))
    ...
    # triplet priors
    embedding_priors = None    
    if self.prior_net is not None:
      #triplet_input = torch.cat([s_vecs_pred, pred_vecs, o_vecs_pred], dim=1)
      prior_input = masks_pred.view(-1,1,self.mask_size*self.mask_size) 
      embedding_priors = self.prior_net(prior_input) 
  
    # triplet binary masks
    triplet_masks_pred = None
    if self.triplet_mask_net is not None:
      # use object mask embedding as input to triplet mask net
      if self.masks_to_triplet_mlp and embedding_priors is not None:
        s_priors, o_priors = embedding_priors[s], embedding_priors[o]
        triplet_input = torch.cat([s_priors.squeeze(), o_priors.squeeze(), s_vecs_pred, pred_vecs, o_vecs_pred], dim=1)
      # directly use mask pixels
      elif self.masks_to_triplet_pixels:
        s_masks_pred, o_masks_pred = masks_pred[s], masks_pred[o]
        s_masks_pred, o_masks_pred = s_masks_pred.view(-1,1,self.mask_size*self.mask_size), o_masks_pred.view(-1,1,self.mask_size*self.mask_size)
        triplet_input = torch.cat([s_masks_pred.squeeze(), o_masks_pred.squeeze(), s_vecs_pred, pred_vecs, o_vecs_pred], dim=1)
      else:
        s_ep_pred, o_ep_pred = extreme_pts_pred[s], extreme_pts_pred[o]
        triplet_input = torch.cat([s_ep_pred, o_ep_pred, s_vecs_pred, pred_vecs, o_vecs_pred], dim=1) 
    ...
    ## save_val_results  return img, boxes_pred, masks_pred, objs, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, rel_scores, obj_vecs, pred_vecs, triplet_boxes_pred, triplet_boxes_gt, triplet_masks_pred
    return img, boxes_pred, masks_pred, objs, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, rel_scores, obj_vecs, pred_vecs, triplet_boxes_pred, triplet_boxes_gt, triplet_masks_pred, boxes_pred_info, triplet_superboxes_pred, extreme_pts_pred
