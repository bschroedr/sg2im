...
def get_gan_losses(gan_type):
  ...
  elif gan_type == 'matching_aware_gan':
    return gan_g_matching_aware_loss, gan_d_matching_aware_loss
  else:
...
def gan_g_matching_aware_loss(scores_fake):
  """
  Input:
  - scores_fake: Tensor of shape (N,) containing scores for fake samples
   Output:
  ...
  if scores_fake.dim() > 1:
    scores_fake = scores_fake.view(-1)
  y_fake = _make_targets(scores_fake, 1)
  loss_real = bce_loss(scores_fake, y_fake)
  return loss_real.sum()

def gan_d_matching_aware_loss(scores_real, scores_fake):
  """
  Input:
  - scores_real: Tensor of shape (N,) giving scores for real samples
  - scores_fake: Tensor of shape (N1,) giving scores for fake samples
   Output:
  - loss: Tensor of shape () giving GAN discriminator loss
  """
  if scores_real.dim() > 1:
  ...
  loss_real = bce_loss(scores_real, y_real)
  loss_fake = bce_loss(scores_fake, y_fake)
  return loss_real.sum() + loss_fake.sum() 
...
def custom_huber_loss(input, target, delta=1):
  diff = (input - target)
  #pdb.set_trace()
  # if |diff| <= delta:
  L = 0.5 * (diff**2)
  ...
  L[idx] = 0
  # if |diff| > delta
  L2 = delta * torch.abs(diff) + - 0.5 * (delta**2)
  L2[~idx] = 0
  L += L2 
  # return mean error 
  return torch.mean(L)
