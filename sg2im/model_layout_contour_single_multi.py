...
class Sg2ImModel(nn.Module):
  ...
  def forward(self, objs, triples, obj_to_img=None,
    ...
    if self.triplet_mask_net is not None:
      ...
      #triplet_masks_pred = triplet_mask_scores.squeeze(1).sigmoid() # only for binary CE loss! 
      triplet_masks_pred = triplet_mask_scores.squeeze(1) # for multi-class CE loss
