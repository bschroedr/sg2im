...
class CondGANDiscriminator(nn.Module):
  def __init__(self, arch, normalization='batch', activation='leakyrelu-0.2',
               padding='same', pooling='avg', input_size=(128,128),
               layout_dim=0,
               sg_context_dim=0):
    ...
    self.cnn = nn.Sequential(cnn, GlobalAvgPool(), nn.Linear(output_dim, 1024))
  ...
  def forward(self, x, layout=None):
    if layout is not None:
      x = torch.cat([x, layout], dim=1)
    vecs = self.cnn(x)  
    real_scores = self.classifier(vecs) 
    return real_scores 
