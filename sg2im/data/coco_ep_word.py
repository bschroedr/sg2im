...
# GLOVE 
from sg2im.load_words import create_vocab_list, load_all_embs, get_vocab_word, get_embs
...
class CocoSceneGraphDataset(Dataset):
  def __init__(self, image_dir, instances_json, stuff_json=None, extreme_pts_json=None,
    ...
    - extreme_pts_json : (optional) Path to a JSON file giving COCO-instances extreme points annotations
    ...
    self.glove_file = 'sg2im/data/word_emb/glove.trimmed.300_withnone.npz'
    self.vocab_file = 'sg2im/data/word_emb/vocab.txt'
    self.all_embeddings = load_all_embs(self.glove_file)
    self.word_emb_size = self.all_embeddings.shape[1]
    self.all_vocab_list = create_vocab_list(self.vocab_file)
    ###
    ...
    self.vocab = {
      'object_name_to_idx': {},
      'pred_name_to_idx': {},
      'object_idx_to_supercat': {} # supercategory
    }
    ...
    for category_data in instances_data['categories']:
      ...
      supercategory_name = category_data['supercategory']
      all_instance_categories.append(category_name)
      object_idx_to_name[category_id] = category_name
      self.vocab['object_name_to_idx'][category_name] = category_id
      # supercategory
      self.vocab['object_idx_to_supercat'][category_id] = supercategory_name
    ...
    if stuff_data:
      ...
      for object_data in stuff_data['annotations']:
        ...
        W, H = self.image_id_to_size[image_id]
        box_area = (w * h) / (W * H)
        ## add trivial extreme points for stuff
        extreme_points = [[x+w/2, y], [x, y+h/2], [x+w/2, h+y], [x+w, y+h/2]]
  ...
  def __getitem__(self, index):
    ...
    for object_data in self.image_id_to_objects[image_id]:
      ...
      # supercategory
      obj_supercats.append(self.vocab['object_idx_to_supercat'][object_data['category_id']])
      x, y, w, h = object_data['bbox']
      ...
      #### normalized extreme points here
      top, left, bottom, right = object_data['extreme_points']
      tt0 = top[0]/WW
      tt1 = top[1]/HH
      ll0 = left[0]/WW
      ll1 = left[1]/HH
      ...
      cx = (ll0 + rr0)/2
      cy = (tt1 + bb1)/2
      extreme_points.append(torch.FloatTensor([tt0,tt1,ll0,ll1,bb0,bb1,rr0,rr1,cx,cy]))
      ########################################
      ...
      # GLOVE word embedding 
      category_name = self.vocab['object_idx_to_name'][object_data['category_id']]
      vocab_word, v_idx = get_vocab_word(category_name, self.all_vocab_list)
      word_emb_vec = get_embs(v_idx, self.all_embeddings)
      #print(word_emb_vec)
      obj_cat_word_embeddings.append(torch.from_numpy(word_emb_vec.astype(np.float32)))
      ...
      ## do the extraction based on extreme points, segmentation masks are tighter than bb mask
      mx0, mx1 = int(round(left[0])), int(right[0])
      my0, my1 = int(round(top[1])), int(bottom[1])
      mx1 = max(mx0 + 1, mx1)
      my1 = max(my0 + 1, my1)
      ######################################
    ...
    if self.heuristics_ordering:
      ...
      for j in range(len(objs_ordered_indx)):
        ...
        new_obj_supercats.append(obj_supercats[j])
      objs = new_objs
      boxes = new_boxes
      masks = new_masks
      # triplet boxes/masks
      ...
      obj_supercats = new_obj_supercats # supercategories
    ...
    # GLOVE word embeddings
    obj_cat_word_embeddings.append(torch.zeros(self.word_emb_size))
    obj_cat_ids.append(torch.tensor(0))
    obj_supercats.append('other')
    # extreme points
    extreme_points.append(torch.FloatTensor([0.5,0,0,0.5,0.5,1,1,0.5,0.5,0.5])) 
    ...
    # GLOVE word embeddings
    obj_cat_word_embeddings = torch.stack(obj_cat_word_embeddings, dim=0)
    obj_cat_ids = torch.stack(obj_cat_ids, dim=0)
    # supercategories
    #obj_supercats = torch.stack(obj_supercats, dim=0)
    # extreme points
    extreme_points = torch.stack(extreme_points, dim=0)
    ...
    return image, objs, boxes, masks, triples, triplet_masks, extreme_points, obj_cat_word_embeddings, obj_cat_ids, obj_supercats
...
def coco_collate_fn(batch):
  ...
  """
  all_imgs, all_objs, all_boxes, all_masks, all_triples, all_triplet_masks, all_extreme_points, all_cat_words, all_cat_ids, all_supercats = [], [], [], [], [], [], [], [], [], []
  #all_imgs, all_objs, all_boxes, all_masks, all_triples = [], [], [], [], []
  all_obj_to_img, all_triple_to_img = [], []
  obj_offset = 0
  # update anything returned by __get_item__ here
  for i, (img, objs, boxes, masks, triples, triplet_masks, extreme_points, cat_words, cat_ids, supercats) in enumerate(batch):
  #for i, (img, objs, boxes, masks, triples) in enumerate(batch):
    ...
    # GLOVE word embeddings
    all_cat_words.append(cat_words)
    all_cat_ids.append(cat_ids)
    #all_supercats += supercats # list
  ...
  all_extreme_points = torch.cat(all_extreme_points)
  all_cat_words = torch.cat(all_cat_words)
  all_cat_ids = torch.cat(all_cat_ids)

  out = (all_imgs, all_objs, all_boxes, all_masks, all_triples,
         all_obj_to_img, all_triple_to_img, all_triplet_masks, all_extreme_points, all_cat_words, all_cat_ids)
  #out = (all_imgs, all_objs, all_boxes, all_masks, all_triples,
  #       all_obj_to_img, all_triple_to_img)
  return out
