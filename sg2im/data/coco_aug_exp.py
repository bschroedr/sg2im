...
class CocoSceneGraphDataset(Dataset):
  def __init__(self, image_dir, instances_json, stuff_json=None,
    ...
    # triplet augmentation
    self.triplet_augmentation = True
    if self.triplet_augmentation:
      print('TRIPLET AUGMENTATION turned on.')
  ...
  def __getitem__(self, index):
    ...
    for cur in real_objs:
      ...
      #  p = 'left of'
      elif theta >= 3 * math.pi / 4 or theta < math.pi: # new
        p = 'upper left of'
      elif theta >= math.pi or theta <= -3 * math.pi / 4: # new
        p = 'lower left of'
      elif -3 * math.pi / 4 <= theta < -math.pi / 4:
        p = 'above'
      #elif -math.pi / 4 <= theta < math.pi / 4:
      #  p = 'right of'
      elif -math.pi / 4 <= theta < 0: # new
        p = 'lower right of'
      elif 0 <= theta < math.pi / 4: # new
        p = 'upper right of'
