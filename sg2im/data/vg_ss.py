...
class VgSceneGraphDataset(Dataset):
  def __init__(self, vocab, h5_path, image_dir, image_size=(256, 256),
               ...
               size_attribute_len=10, grid_size=25):
  ...
  def __getitem__(self, index):
    ...
    # set object attributes for dummy object
    #attrs[i] = self.data['object_attributes'][index, obj_idx]
    # set size attribute
    ...
    # size and location attributes
    spatial_attrs = torch.cat([size_attribute, location_attribute], dim=1)
    return image, objs, boxes, triples, num_attrs, attrs, spatial_attrs
...
def vg_collate_fn(batch):
  ...
  all_imgs, all_objs, all_boxes, all_triples, all_num_attrs, all_attrs, all_sp_attrs = [], [], [], [], [], [], [], 
  all_obj_to_img, all_triple_to_img = [], []
  obj_offset = 0
  for i, (img, objs, boxes, triples, num_attrs, attrs, sp_attrs) in enumerate(batch):
  ...
  all_sp_attrs = torch.cat(all_sp_attrs)
  all_obj_to_img = torch.cat(all_obj_to_img)
  all_triple_to_img = torch.cat(all_triple_to_img)

  out = (all_imgs, all_objs, all_boxes, all_triples,
         all_obj_to_img, all_triple_to_img, all_num_attrs, all_attrs, all_sp_attrs)
  return out
