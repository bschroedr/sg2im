...
class VgSceneGraphDataset(Dataset):
  ...
  def __getitem__(self, index):
    ...
    return image, objs, boxes, triples, num_attrs, attrs
...
def vg_collate_fn(batch):
  ...
  all_imgs, all_objs, all_boxes, all_triples, all_num_attrs, all_attrs = [], [], [], [], [], [], 
  all_obj_to_img, all_triple_to_img = [], []
  obj_offset = 0
  for i, (img, objs, boxes, triples, num_attrs, attrs) in enumerate(batch):
  ...
  out = (all_imgs, all_objs, all_boxes, all_triples,
         all_obj_to_img, all_triple_to_img, all_num_attrs, all_attrs)
  return out
