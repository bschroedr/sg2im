...
from sg2im.data.coco_contour_debug import CocoSceneGraphDataset, coco_collate_fn
...
from sg2im.losses import get_gan_losses, custom_huber_loss, custom_huber_loss_test
from sg2im.metrics import jaccard, relation_score

from sg2im.model_layout_contour_single import Sg2ImModel
...
def parse_args():
  ...
  parser.add_argument('--triplet_contours_pred_loss_weight', default=10, type=float)
  parser.add_argument('--triplet_mask_loss_weight', default=10.0, type=float)
  parser.add_argument('--contours_pred_loss_weight', default=10, type=float)
  ...
  parser.add_argument('--huber_loss_bbox', default=False, type=int)
  parser.add_argument('--huber_loss_contour', default=False, type=int)
  ...
  parser.add_argument('--check_val_metrics_every', default=5000, type=int)
...
def build_img_discriminator(args, vocab):
  ...
  if args.sg_context_dim_d > 0:  
    ...
    if args.image_patch_discr:
      discriminator = CondGANPatchDiscriminator(**d_kwargs, layout_dim=layout_dim, sg_context_dim = args.sg_context_dim_d)
    else:
      discriminator = CondGANDiscriminator(**d_kwargs, layout_dim=layout_dim, sg_context_dim = args.sg_context_dim_d)   
  else:
...
def build_coco_dsets(args):
  ...
  # to deactivate randomization for val (for consistent results)
...
def min_max_bbox_fr_contours(contours_pred):
  cp = contours_pred.view(-1,12,2)
  min_pts, _ = cp.min(1)  # second val is indices
  max_pts, _ = cp.max(1)
  boxes_pred = torch.cat([min_pts, max_pts], dim=1)
  return boxes_pred
...
def get_rel_score(args, t, loader, model):
  ...
  with torch.no_grad():
    ...
    for batch in loader:
      ...
      #imgs_pred, boxes_pred, masks_pred, objs_vec, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, predicate_scores, obj_embeddings, pred_embeddings, triple_boxes_pred, triple_boxes_gt, triplet_masks_pred, triplet_contour_pred = model_out
      ...
      if contours_pred is not None and boxes_pred is None:
        boxes_pred = min_max_bbox_fr_contours(contours_pred)
...
def check_model(args, t, loader, model, logger=None, log_tag='', write_images=False):
  ...
  with torch.no_grad():
    for batch in loader:
      ...
      model_out = model(objs, triples, obj_to_img, boxes_gt=boxes, masks_gt=model_masks)
      # imgs_pred, boxes_pred, masks_pred, predicate_scores = model_out
      #imgs_pred, boxes_pred, masks_pred, objs_vec, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, predicate_scores, obj_embeddings, pred_embeddings, triplet_boxes_pred, triplet_boxes, triplet_masks_pred = model_out
      imgs_pred, boxes_pred, masks_pred, objs_vec, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, predicate_scores, obj_embeddings, pred_embeddings, triplet_boxes_pred, triplet_boxes, triplet_masks_pred, triplet_contours_pred, contours_pred = model_out
      ...
      total_loss, losses =  calculate_model_losses(
                                ...
                                triplet_contours, triplet_contours_pred,
                                contours, contours_pred,
...
def calculate_model_losses(args, skip_pixel_loss, model, img, img_pred,
  ...
  # individual bounding box regression loss 
  if args.bbox_pred_loss_weight > 0 and bbox is not None and bbox_pred is not None:
    if args.huber_loss_bbox:
      loss_bbox = custom_huber_loss_test(bbox_pred.cuda(), bbox, delta=args.huber_loss_delta)
      #loss_bbox = custom_huber_loss(bbox_pred.cuda(), bbox, delta=args.huber_loss_delta)
      #loss_bbox = F.smooth_l1_loss(bbox_pred.cuda(), bbox)
    else:
      loss_bbox = F.mse_loss(bbox_pred.cuda(), bbox)
    total_loss = add_loss(total_loss, loss_bbox, losses, 'bbox_pred',
                          args.bbox_pred_loss_weight)

  # 8 point bounding box regression for triplets
  ...
  # contours  regression for objects 
  if args.contours_pred_loss_weight > 0 and contours is not None and contours_pred is not None:
    if args.huber_loss_contour:
      # delta: controls balance of L1 and L2 loss (default: delta = 1, larger use more L2, smaller use more L1)
      loss_contours = custom_huber_loss_test(contours_pred, contours, delta=args.huber_loss_delta)
      #loss_contours = custom_huber_loss(contours_pred, contours, delta=args.huber_loss_delta)
      #loss_contours = F.smooth_l1_loss(contours_pred, contours)
    else:
      loss_contours = F.mse_loss(contours_pred, contours)
    total_loss = add_loss(total_loss, loss_contours, losses, 'contours_pred', 
                          args.triplet_contours_pred_loss_weight)

  # contours  regression for triplets
  if args.triplet_contours_pred_loss_weight > 0 and triplet_contours is not None and triplet_contours_pred is not None:
    loss_triplet_contours = F.mse_loss(triplet_contours_pred, triplet_contours)
    total_loss = add_loss(total_loss, loss_triplet_contours, losses, 'triplet_contours_pred', 
                        args.triplet_contours_pred_loss_weight)
...
def main(args):
  ...
  restore_path = None
  if args.checkpoint_start_from is not None:
    # this should give path to model to be compatible with bu
    restore_path = args.checkpoint_start_from
    #assert os.path.isfile(args.checkpoint_start_from), print(args.checkpoint_start_from, 'not found')
    ...
    for batch in train_loader:
      ...
      if contours_pred is not None and boxes_pred is None:
        ...
        if 0: 
          ...
          ax.scatter(bb[:,0],bb[:,1], linewidth=0.5)
          ax.scatter(bbp[:,0],bbp[:,1], linewidth=0.5)
          plt.show()
          # display masks
