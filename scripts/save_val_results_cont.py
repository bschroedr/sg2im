...
from sg2im.metrics import jaccard, relation_score, relation_score_cont


from sg2im.model_layout_contour_single import Sg2ImModel
#from sg2im.model_layout_mask import Sg2ImModel
...
def min_max_bbox_fr_contours(contours_pred):
  ...
  return boxes_pred, mean_pts, std_pts
...
def get_rel_score(args, t, loader, model):
  ...
  with torch.no_grad():
	...
	# get min-max bboxes, contour mean (not bbox mean), contour std dev (e.g. use predicted bboxes to estimate centers)
        boxes_pred, mean_pts_pred, std_pts_pred = min_max_bbox_fr_contours(contours_pred) # predicted
