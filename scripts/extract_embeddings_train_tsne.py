      ...
      for batch in loader:
        ...
        for i in range(0,num_batch_samples):
          ...
          for j in range(0, num_objs):
            name = obj_names[j]
            entry = {'id': objs_img[j].tolist(), 'embed': obj_embeddings_img[j].tolist(), 'word_embed': obj_embeddings_word[j].tolist()}
...
def analyze_hierarchical_clustering(mean_embed, word_embed, class_ids, class_labels):
...
def analyze_object_db(db, analyze_word_embed=False):
  ...
  for n in range(0,len(sort_objs_by_count)):
    ...
    word_embeds += [o[l]['word_embed'] for l in range(0,len(o))]
  ...
  pdb.set_trace()
  # save data to file
  out_dir = '/nfs/site/home/brigitsc/layout_models/'
  np.savetxt((out_dir + 'all_train_embed.txt'), X)
  np.savetxt((out_dir +'all_train_ids.txt'), y)
  np.savetxt((out_dir + 'all_train_word_embed.txt'), np.array(word_embeds)) 
  ...
  if analyze_word_embed:
    ...
    analyze_hierarchical_clustering(mean_embed, word_embeds, mean_ids, sort_objs_by_count)
