...
COCO_DIR = os.path.expanduser('/home/brigit/sandbox/sg2im/datasets/coco')
...
def parse_args():
  ...
  parser.add_argument('--dataset', default='vg', choices=['vg', 'coco'], help='Dataset to use')
...
def get_rel_score(args, t, loader, model):
  ...
  with torch.no_grad():
    ...
    for batch in loader:
      ...
      model_masks = masks
      model_out = model(objs, triples, obj_to_img, boxes_gt=boxes, spatial_attributes=sp_attrs, masks_gt=model_masks, tr_to_img=triple_to_img)
...
def create_pred_onehots(predicates, num_cats):
    ...
    for batch in train_loader:
      ...
      with timeit('forward', args.timing):
        ...
        model_out = model(objs, triples, obj_to_img,
                          boxes_gt=model_boxes, spatial_attributes=sp_attrs, masks_gt=model_masks, tr_to_img=triple_to_img
                          )
        # imgs_pred, boxes_pred, masks_pred, predicate_scores = model_out
        imgs_pred, boxes_pred, masks_pred, objs_vec, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, predicate_scores, obj_embeddings, pred_embeddings, triplet_boxes_pred, triplet_boxes, triplet_masks_pred, boxes_pred_info, triplet_superboxes_pred, obj_scores, pred_mask_gt, pred_mask_scores, context_tr_vecs, input_tr_vecs, obj_class_scores, rel_class_scores, subj_scores, rel_embedding, mask_rel_embedding, pred_ground = model_out
