...
# 59% model
#from sg2im.model_layout_contour_8pt_dbug import Sg2ImModel
...
def check_model(args, t, loader, model, log_tag='', write_images=False):
  ...
  with torch.no_grad():
	...
	# need to iterate over all triples due to information that needs to be extracted per triple
      ...
      # object masks
      np_masks_pred = [mask.cpu().numpy() for mask in masks_pred] # # objects
      np_masks = [mask.cpu().numpy() for mask in model_masks] # # objects
      np_layout_masks = [mask.cpu().numpy() for mask in layout_masks] # # objects
      ...
      layouts = vis.debug_layout_mask(model.vocab, objs, boxes_pred, layout_masks, obj_to_img, W=256, H=256)
     ...
     # visualize GT boxes/images
      ...
      for o in obj_contours:
        ...
        ax.scatter(oc[:,0], oc[:,1], linewidth=0.5) # order was switched in coco_cont.py
        ax.scatter(ocp[:,0], ocp[:,1], linewidth=0.5) # order was switched in coco_cont.py
        plt.show()
        #pdb.set_trace()
        c += 1
      ...
      if write_images:
        t = 0
        for gt_img, pred_overlaid_img, overlaid_img, layout_img in zip(np_imgs, pred_overlaid_images, overlaid_images, layouts):
