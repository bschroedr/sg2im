...
def build_model(args, vocab):
  ...
  if args.checkpoint_start_from is not None:
    ...
    # model vocal should be VG not COCO 
    kwargs['vocab'] = vocab
    model = Sg2ImModel(**kwargs) 
    model_dict = model.state_dict() 
    # load subnetwork
    pretrained_dict = checkpoint['model_state'] # state dictionary for checkpoint model 
    # find subnet keys, values
    filtered_dict = {}
    ...
    # overwrite entries in existing model state dictionary  with those from subnet
  ...
  with torch.no_grad():
    for batch in loader:
      ...
      if args.triplet_superbox_net and triplet_superboxes_pred is not None:
        ...
        tb = triplet_boxes
        # get super box
        min_x = np.min([tb[:,0].cpu().numpy(), tb[:,4].cpu().numpy()], axis=0)
        min_y = np.min([tb[:,1].cpu().numpy(), tb[:,5].cpu().numpy()], axis=0)
...
def calculate_model_losses(args, skip_pixel_loss, model, img, img_pred,
  ...
  # point bounding box regression for triplets
...
def main(args):
  ...
  if restore_path is not None and os.path.isfile(restore_path):
    ...
    # disable this when finetuning from a COCO model
