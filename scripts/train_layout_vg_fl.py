...
from sg2im.focalloss import FocalLossBinary
...
def calculate_model_losses(args, skip_pixel_loss, model, img, img_pred,
  ...
  if args.predicate_pred_loss_weight > 0:
    pred_onehots = create_pred_onehots(predicates, len(predicate_scores[0]))
    #predicate_scores = F.sigmoid(predicate_scores)
    #loss_predicate = F.binary_cross_entropy(predicate_scores, pred_onehots)
    loss_predicate = FocalLossBinary(gamma=2.0, alpha=0.75)(predicate_scores, pred_onehots)
