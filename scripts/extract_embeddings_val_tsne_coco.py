    ...
    with torch.no_grad():
      for batch in loader:
        ...
        if len(batch) == 6: # VG
          ...
          imgs, objs, boxes, masks, triples, obj_to_img, triple_to_img, triplet_masks, one_hots = batch
        predicates = triples[:, 1]
  
        # Run the model as it has been run during training
        model_masks = masks
        model_out = model(objs, triples, obj_to_img, boxes_gt=boxes, masks_gt=model_masks, one_hot_vecs=one_hots)
        ...
        imgs_pred, boxes_pred, masks_pred, objs_vec, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, predicate_scores, obj_embeddings, pred_embeddings, triple_boxes_pred, triple_boxes_gt, triplet_masks_pred, boxes_pred_info, triplet_superboxes_pred, one_hot_masks_pred = model_out
        ...
        imgs = imgs.detach()
        # not necessary since done in VG data loader
        #samples = {}
        ...
        for i in range(0,num_batch_samples):
          ...
          objs_img = objs[objs_index].cpu() # object class id labels for image
          obj_names = np.array(model.vocab['object_idx_to_name'])[objs_img] # index with object class index
          obj_one_hot_masks = one_hot_masks_pred[objs_index].cpu()
          ...
          for j in range(0, num_objs):
            ...
            print(' >>> ', name)
            mask_int = (255.0 * obj_one_hot_masks[j]).numpy().astype(int)
            mask_int = (mask_int > 128).astype(np.int64)
            plt.imshow(mask_int)
            plt.show()
