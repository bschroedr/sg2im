...
def parse_args():
  ...
  parser.add_argument('--triplet_mask_loss_weight', default=1.0, type=float)
...
def check_model(args, t, loader, model, logger=None, log_tag='', write_images=False):
  ...
  with torch.no_grad():
    for batch in loader:
      ...
      imgs_pred, boxes_pred, masks_pred, objs_vec, layout, layout_boxes, layout_masks, obj_to_img, sg_context_pred, sg_context_pred_d, predicate_scores, obj_embeddings, pred_embeddings, triplet_boxes_pred, triplet_boxes, triplet_masks_pred, triplet_contours_pred = model_out
...
def calculate_model_losses(args, skip_pixel_loss, model, img, img_pred,
  ...
  if args.triplet_bboxes_pred_loss_weight > 0 and triplet_boxes is not None and triplet_boxes_pred is not None:
